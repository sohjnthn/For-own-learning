...
Conclusion: It is better to use IntelliJ IDEA - Java - build.gradle (Gradle DSL as Groovy) - opendsk-23, to test automation for the desktop web browser; and for the real mobile phone device, for the related separate Cucumber feature files; or for the testng.xml which tests for the main Java classes of the same package folder. There would not be difficulties for adding or removing dependencies (Android Studio); some configurations which cannot run (Eclipse IDE for Java, cannot run Cucumber feature files); or being unable to run driver.get("...) code lines (IntelliJ IDEA - Java - Maven project).
