    ...
    public ThirdAppObjects(AndroidDriver driver, ExtentTest extentTest, ExtentSparkReporter extentSparkReporter, ExtentReports extentReport) throws IOException {
        ...
        for(int i=0; i<10; i++) {
            if (Objects.equals(oneString, "1")) {
                ...
                extentTest.addScreenCaptureFromPath(capturedScreenshotImageFilepathString).pass(MediaEntityBuilder.createScreenCaptureFromPath(capturedScreenshotImageFilepathString).build()).log(Status.PASS, "The app requires time to load.");
                System.out.println("Pass");
            } else {
                capturedScreenshotImageFilepathString = takeScreenshot(driver);
                extentTest.addScreenCaptureFromPath(capturedScreenshotImageFilepathString).pass(MediaEntityBuilder.createScreenCaptureFromPath(capturedScreenshotImageFilepathString).build()).log(Status.FAIL, "The app requires time to load.");
        ...
        // We let the system take 10 more screenshot images
        for(int i=0; i<10; i++) {
            if (Objects.equals(oneString, "1")) {
                capturedScreenshotImageFilepathString = takeScreenshot(driver);
                extentTest.addScreenCaptureFromPath(capturedScreenshotImageFilepathString).pass(MediaEntityBuilder.createScreenCaptureFromPath(capturedScreenshotImageFilepathString).build()).log(Status.PASS, "The required search results webpage's search result would be clicked on.");
                System.out.println("Pass");
            } else {
                capturedScreenshotImageFilepathString = takeScreenshot(driver);
                extentTest.addScreenCaptureFromPath(capturedScreenshotImageFilepathString).pass(MediaEntityBuilder.createScreenCaptureFromPath(capturedScreenshotImageFilepathString).build()).log(Status.FAIL, "The required search results webpage's search results would not not be clicked on.");
                System.out.println("Fail");
            }
        }
